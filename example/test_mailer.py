...
class TestMailerStdout(unittest.TestCase):
    '''
    A test fixture which tests mock_mailer.py with stdout.
    '''
    ...
    def setUp(self):
        ...
        self.__process = subprocess.Popen(
            args=('python', '-B', self.__data_dir + os.sep + 'mock_mailer.py'),
    ...
    def test_typical(self):
        ...
        s_verifier.assertPattern('Mailer start')
        s_verifier.assertPattern('Wait for user interaction')
        s_verifier.assertPattern('Exit')

        # This test expects that the process exits in 10 seconds from outputting 'Exit' and exit code is 0.
    ...
    def test_fail(self):
        ...
        with self.assertRaises(AssertionError):
            # Following assertion raises AssertionError.
            s_verifier.assertPattern('Does not occur', timeout = 2)

        # terminate quickly
        with self.assertRaises(AssertionError):
            # Following assertion raises AssertionError.
            p_verifier.assertExit(timeout = 0)


class TestMailerLogfile(unittest.TestCase):
    '''
    A test fixture which tests mock_mailer.py with logfile.
    '''
    ...
    def test_typical(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                self.__process = subprocess.Popen(
                    args=('python', '-B', self.__data_dir + os.sep + 'mock_mailer.py', '-l', path_logfile),
